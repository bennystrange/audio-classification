...
def extract_features(file_path, standard_audio_size=4):
    try:
        # Load the audio file
        audio_timeseries, sampling_rate = librosa.load(file_path, sr=None)
        padded_audio = librosa.util.fix_length(audio_timeseries, size=standard_audio_size*sampling_rate) # array size is required_audio_size*sampling frequency

        # Extract features
        fft_freqs = librosa.fft_frequencies(sr=sampling_rate, n_fft=512)
        ...
        #all_features = np.vstack([fft_freqs, mfccs])
        all_features = fft_freqs
...
for label in labels:
    for num, file_name in enumerate(os.listdir(f"/content/drive/My Drive/378final/{label}")):
        if (num < 100): # remove this to read ALL files
            if num % 10 == 0:
                print(num)
            file_path = f"/content/drive/My Drive/378final/{label}/{file_name}"
...
for i, file_name in enumerate(os.listdir(f"/content/drive/My Drive/378final/Test")):
    if i % 10 == 0:
        print(i)
    file_path = f"/content/drive/My Drive/378final/Test/{file_name}"
